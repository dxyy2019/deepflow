...
type ResourcePubSub[
	...
	PublishBatchDeleted(int, MDPT) // publish resource batch deleted notification, including specific data
}
...
func (p *ResourcePubSubComponent[MAPT, MAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT]) PublishBatchDeleted(orgID int, msg MDPT) {
	// log.Infof("publish delete %#v", msg)
	for topic, subs := range p.subscribers {
		if topic == TopicResourceBatchDeletedLcuuid {
			for _, sub := range subs {
				sub.(ResourceBatchDeletedSubscriber).OnResourceBatchDeleted(orgID, msg.GetLcuuids())
			}
		}
		if topic == TopicResourceBatchDeletedMySQL {
			for _, sub := range subs {
				sub.(ResourceBatchDeletedSubscriber).OnResourceBatchDeleted(orgID, msg.GetMySQLItems())
			}
		}
	}
}
